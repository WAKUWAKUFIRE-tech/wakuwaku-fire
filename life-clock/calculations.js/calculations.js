...
// Monthly withdrawals are the same recurrence used by the asset projection.
export function calculateDailyLivingBudget(assets, rate, days) {
  if (!Number.isFinite(assets) || !Number.isFinite(rate) || !Number.isFinite(days) || assets <= 0 || days <= 0) return 0;
  const months = days / DAYS_PER_YEAR * 12;
  const monthlyRate = Math.expm1(Math.log1p(rate / 100) / 12);
  if (Math.abs(monthlyRate) < 1e-12) return assets / days;
  const monthlyWithdrawal = assets * monthlyRate / -Math.expm1(-months * Math.log1p(monthlyRate));
  return Math.max(0, monthlyWithdrawal * 12 / DAYS_PER_YEAR);
}
export function calculateRequiredLifetimeAssets(spending, rate, days) {
  if (!Number.isFinite(spending) || !Number.isFinite(rate) || !Number.isFinite(days) || spending <= 0 || days <= 0) return 0;
  const months = days / DAYS_PER_YEAR * 12;
  const monthlyRate = Math.expm1(Math.log1p(rate / 100) / 12);
  const monthlySpending = spending / 12;
  if (Math.abs(monthlyRate) < 1e-12) return monthlySpending * months;
  return Math.max(0, monthlySpending * -Math.expm1(-months * Math.log1p(monthlyRate)) / monthlyRate);
}
export function calculateWakuwakuSurplus(assets, requiredLifetimeAssets) {
  if (!Number.isFinite(assets) || !Number.isFinite(requiredLifetimeAssets)) return 0;
  return Math.max(0, assets - requiredLifetimeAssets);
}
export function calculateDailyWakuwakuBudget(surplus, healthyDays) {
  if (!Number.isFinite(surplus) || !Number.isFinite(healthyDays) || surplus <= 0 || healthyDays <= 0) return 0;
  return surplus / healthyDays;
}
export function calculateCurrentDailyLivingCost(spending) {
  return Number.isFinite(spending) && spending > 0 ? spending / DAYS_PER_YEAR : 0;
}
