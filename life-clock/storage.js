...
export function validateState(data) {
  ...
  if (!Array.isArray(data.bucketList) || data.bucketList.some(item => typeof item.id !== 'string' || typeof item.title !== 'string' || !item.title.trim() || item.title.length > 120 || typeof item.dueDate !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(item.dueDate) || typeof item.done !== 'boolean' || (item.categoryId !== undefined && (typeof item.categoryId !== 'string' || !item.categoryId.trim() || item.categoryId.length > 80)) || (item.categoryName !== undefined && (typeof item.categoryName !== 'string' || item.categoryName.length > 120)))) throw new Error('保存しているバケットリストを読み取れません。');
