...
`/otoku/` の案件データには、取得元の公式URLをそのまま保存しています。A8.netのリンクマネージャー正式タグが設定された場合だけ、ページの公式リンクをブラウザ側で自動変換します。タグが無い場合や対象外の広告主の場合も、公式URLで正常に開けます。

提携後の収益案件は `config/affiliate-offers.json` に一度登録すると、案件のジャンルに応じて関連カードへ自動表示されます。未登録・無効・期限外の案件は空の広告枠を表示しません。Amazonは個別商品を選ばず、セールページへの導線を1ページ1回だけ表示し、正式なアソシエイトURLが設定されたときだけ収益リンクとして扱います。

`automation/otoku/affiliate-analysis.json` は、その日の掲載案件をジャンル別に集計した確認用データです。提携候補は `automation/otoku/affiliate-candidates.json` にまとめてあり、候補URLやIDを推測して登録することはありません。日々の案件取得・ジャンル判定・ページ生成・期限切れ除外は既存の毎朝更新に含まれるため、毎日広告URLを貼り替える作業は不要です。
...
ランキングURLの設定場所は **`config/external-ranking.js`** です。
「ブログ村URLを設定して：取得したURL」とCodexに伝えるだけで変更できます。
`blogmura.url` に本人専用のHTTPS投票URLを設定すると記事末尾に表示されます。空URLは非表示、`enabled: false` で休止できます。トップには表示しません。今回のランキング対応はブログ村のみです。

RSSは `/feed.xml`、サイトマップは `/sitemap.xml`。記事自動公開時に再生成します。手動記事追加・更新後は `npm run build:growth` を実行してから公開してください。設定ファイルだけのURL変更は再生成不要です。

本人が外部サービスで行うクリック順と検証項目は [外部サービス登録チェックリスト](docs/external-growth-checklist.md) を参照してください。
