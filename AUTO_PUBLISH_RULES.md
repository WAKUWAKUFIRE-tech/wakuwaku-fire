...
- 事前stock全体の検証では、外部サイトの一時的な応答不良で全記事を巻き込まないよう、出典URLの形式・ドメイン・本文リンクを確認します。公開対象の記事だけは公開処理で追加確認できます。
- 未公開記事が残っているのに公開可能なstockがない場合は、`blocked_no_stock` を記録してActionsを失敗させます。成功扱いで静かに止めず、通知対象にします。
- すべての記事が `published` または `skipped` になった後だけ、stock切れを正常終了とします。
- 個別記事の根拠不足は `needs_review`、技術エラーや公開確認失敗は `failed` として理由を残します。
- 公開確認に失敗した場合は、生成した記事フォルダ・編集メモ・一覧・sitemapをロールバックし、失敗記事の公開情報を空にしてから次回のstock選択へ進めます。失敗した生成物を残して次回検証をslug重複で止めないようにします。
...
既存記事と今後の自動投稿記事は、次の7カテゴリのいずれかに必ず分類します。
...
カテゴリは記事一覧の見出し・記事カード・記事ページのパンくず・構造化データに反映します。
