...
import { DAYS_PER_YEAR, DAY_MS, validateProfile, elapsedDays, calculateRemainingDays, calculateHealthyDays, calculateDailyLifeBudget, calculateAssetLifetime, calculateProjectedAssets, calculateRemainingEvents, calculateProjection } from '../life-clock/calculations.js';
...
test('Life horizon days subtract elapsed time and clamp to zero', () => { assert.equal(calculateRemainingDays(36, 88), 18993); assert.equal(calculateRemainingDays(36, 88, 7), 18986); assert.equal(calculateHealthyDays(36, 75), 14245); assert.equal(calculateHealthyDays(36, 75, 7), 14238); assert.equal(calculateHealthyDays(75, 75), 0); assert.equal(calculateHealthyDays(76, 75), 0); assert.equal(calculateRemainingEvents(calculateHealthyDays(36, 75), 1), 39); });
